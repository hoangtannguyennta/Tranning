    ...
    <div class="container">
        <div class="form">
            ...
            <div class="form-bottom">
                ...
                @endif
                <form action="{{ route('permissionRole.update', $roles_id->id) }}" enctype="multipart/form-data" method="POST">
                    @csrf
                    @method('POST')
                    <div class="form-content">
                        ...
                        <select class="select" name="role_id" disabled>
                            ...
                            @foreach ($roles as $role)
                                <option value="{{ $role->id }}" {{ $roles_id->id == $role->id ? 'selected': '' }} >{{ $role->name }}</option>
                            @endforeach
                        </select>
